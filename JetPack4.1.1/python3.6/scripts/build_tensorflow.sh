...
CUDA_VERSION=10.0
TF_VERSION=1.10
CUDA_COMPUTE_CAPABILITIES=7.2,6.2,5.3 # Xavier:7.2, TX2:6.2, TX1:5.3
TF_NEED_TENSORRT=1 # if TF_VERSION < 1.8, use 0
...
&& $SCRIPT_DIR/patch_tf1.6_cuda10.sh \
...
&& bazel build --config=cuda --config="opt" --copt='-march=native' --copt="-O3" --verbose_failures --subcommands //tensorflow/tools/pip_package:build_pip_package \
